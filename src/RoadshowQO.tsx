...
export default function RoadshowQO() {
  ...
  const handleDownloadPdf = async () => {
    ...
    const proposalElement = document.querySelector<HTMLElement>(".printableArea");

    if (!proposalElement) {
      setPdfError("Unable to find the quotation preview for PDF download.");
      return;
    }
  ...
  const isCompactProposal = quoteLineItems.length >= 5 || Boolean(uploadedSignature);
